...
// const prototypes = [p2,p7,p6,p8,p5,p4,p11,p9,p10,p1,p12,p13,p15,p16];
...
//         {showSignature && (
//           <motion.div
//             className="w-full h-screen bg-white fixed top-0 left-0 flex items-center justify-center z-10000000"
//             initial={{ opacity: 1 }}
//             exit={{ opacity: 0 }}
...
export default function Prototype() {
  ...
  return (
    <div className="w-full bg-white flex flex-col items-center justify-start overflow-hidden pt-10">
      <div
        ref={scrollRef}
        className="flex gap-6 overflow-x-auto w-full px-6 md:px-10 mt-29 py-12 scrollbar-hide"
      >
        {prototypes.map((src, index) => (
          <ZoomImage key={index} src={src} index={index} />
        ))}
...
function ZoomImage({ src, index }) {
  ...
  return (
    <motion.img
      ...
      src={src}
      alt={`Prototype ${index + 1}`}
      className="w-73 h-72 md:w-[340px] h-72 md:h-85 object-cover rounded-[30px] shadow-lg shrink-0"
      initial={{ scale: 0.8, opacity: 0 }}
      animate={isInView ? { scale: 1, opacity: 1 } : {}}
      transition={{ duration: 0.5, delay: index * 0.1 }}
    />
